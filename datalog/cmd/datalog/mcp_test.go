...
func newMordorHandlers(t *testing.T) *mcpHandlers {
	...
	h, closeFn, err := newMCPHandlers(zipPath, "", nil, "", 5*time.Second, defaultMaxFacts, false, true)
	if err != nil {
...
func TestNewMCPHandlers_WarnsOnEmbeddedQueriesInRuleFiles(t *testing.T) {
	...
	h, closeFn, err := newMCPHandlers(dir, "", []string{rulesPath}, "", 5*time.Second, defaultMaxFacts, false, true)
...
func TestQuery_ZeroRulesCrossProductExceedsFactCap(t *testing.T) {
	...
	h, closeFn, err := newMCPHandlers(dir, "", nil, "", 5*time.Second, testFactCap, false, true)
	if err != nil {
		t.Fatalf("newMCPHandlers: %v", err)
	}
	defer closeFn()
...
func TestQuery_DerivationAboveOldHardcodedCapSucceedsUnderNewDefault(t *testing.T) {
	...
	h, closeFn, err := newMCPHandlers(dir, "", nil, "", 5*time.Second, defaultMaxFacts, false, true)
	if err != nil {
		t.Fatalf("newMCPHandlers: %v", err)
	}
	defer closeFn()
...
func TestCheckFactCap_ZeroMaxFactsNeverCaps(t *testing.T) {
	...
	h, closeFn, err := newMCPHandlers(dir, "", nil, "", 5*time.Second, 0, false, true)
	if err != nil {
		t.Fatalf("newMCPHandlers: %v", err)
	}
	defer closeFn()
