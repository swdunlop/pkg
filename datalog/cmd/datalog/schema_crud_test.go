...
func newSchemaCrudTestHandlers(t *testing.T, dataDir string, seedYAML string) (*mcpHandlers, string) {
	...
	h, closeFn, err := newMCPHandlers(dataDir, schemaPath, nil, "", 5_000_000_000, defaultMaxFacts)
	if err != nil {
...
func TestNewMCPHandlers_RejectsDuplicateMatcherKey(t *testing.T) {
	...
	_, _, err := newMCPHandlers(dir, schemaPath, nil, "", 5_000_000_000, defaultMaxFacts)
	if err == nil {
...
func TestPutMatcher_DuplicateKeySeedNeverReachesCorruption(t *testing.T) {
	...
	h, _, err := newMCPHandlers(dir, schemaPath, nil, "", 5_000_000_000, defaultMaxFacts)
	if err == nil {
...
func TestPutMatcher_SurvivesReloadFromDisk(t *testing.T) {
	...
	fresh, closeFn, err := newMCPHandlers(dir, schemaPath, nil, "", 5_000_000_000, defaultMaxFacts)
	if err != nil {
...
func TestSchemaWrite_PreservesFromIndirection(t *testing.T) {
	...
	fresh, closeFn, err := newMCPHandlers(dataDir, schemaPath, nil, "", 5_000_000_000, defaultMaxFacts)
	if err != nil {
		t.Fatalf("newMCPHandlers (fresh): %v", err)
	}
	defer closeFn()
...
func TestPutDeclaration_PreservesContainsFromOnDisk(t *testing.T) {
	...
	fresh, closeFn, err := newMCPHandlers(filepath.Dir(schemaPath), schemaPath, nil, "", 5_000_000_000, defaultMaxFacts)
	if err != nil {
		t.Fatalf("newMCPHandlers (fresh): %v", err)
	}
	defer closeFn()
...
func TestPredicateDeps_DerivedWithGroupAddress(t *testing.T) {
	...
	h, closeFn, err := newMCPHandlers(dataDir, "", nil, rulesDir, 5_000_000_000, defaultMaxFacts)
	if err != nil {
		t.Fatalf("newMCPHandlers: %v", err)
	}
	defer closeFn()
